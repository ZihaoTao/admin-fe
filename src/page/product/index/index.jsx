    ...
    onSetProductStatus(e, productId, currentStatus){
        let newStatus   = currentStatus == 1 ? 2 : 1,
            confrimTips = currentStatus == 1 
                ? 'Are you sure to remove this item?' : 'Are you sure to sell this item?';
                        ...
                        this.state.list.map((product, index) => {
                            return (
                                <tr key={index}>
                                    ...
                                    <td>${product.price}</td>
                                    <td>
                                        <p>{product.status == 1 ? 'On sell' : 'Not Available'}</p>
                                        <button className="btn btn-xs btn-warning" 
                                            onClick={(e) => {this.onSetProductStatus(e, product.id, product.status)}}>{product.status == 1 ? 'Not Available' : 'On sell'}</button>
                                    </td>
                                    <td>
